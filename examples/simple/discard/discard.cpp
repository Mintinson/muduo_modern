#include "discard.hpp"

#include "chaoxi/base/Logging.hpp"
#include "chaoxi/net/EventLoop.hpp"
#include "chaoxi/net/InetAddress.hpp"
#include "chaoxi/net/TcpServer.hpp"

#include <string>

DiscardServer::DiscardServer(chaoxi::net::EventLoop* loop,
                             const chaoxi::net::InetAddress& listenAddr)
    : loop_(loop)
    , server_(loop, listenAddr, "DiscardServer")
{
    server_.setConnectionCallback(
        [this](const chaoxi::net::TcpConnectionPtr& conn)
        { this->onConnection(conn); });

    server_.setMessageCallback(
        [this](const chaoxi::net::TcpConnectionPtr& conn,
               chaoxi::net::Buffer& buf, chaoxi::Timestamp time)
        { this->onMessage(conn, buf, time); });
}

void DiscardServer::start()
{
    server_.start();
}

void DiscardServer::onConnection(const chaoxi::net::TcpConnectionPtr& conn)
{
    FLOG_INFO("DiscardServer - {} -> {} is {}", conn->peerAddress().toIpPort(),
              conn->localAddress().toIpPort(),
              (conn->connected() ? "UP" : "DOWN"));
}

void DiscardServer::onMessage(const chaoxi::net::TcpConnectionPtr& conn,
                              chaoxi::net::Buffer& buf,
                              chaoxi::Timestamp time) {
    std::string msg(buf.retrieveAllAsString());
    LOG_INFO << conn->name() << " discards " << msg.size()
             << " bytes received at "
             << std::format("{:%Y-%m-%d %H:%M:%S}", time);
    // Do nothing with the message, just discard it
}